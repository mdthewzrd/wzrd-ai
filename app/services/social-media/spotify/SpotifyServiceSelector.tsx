...
export default function SpotifyServiceSelector({ services }: Props) {
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {selectedService.packages.map((pkg) => (
              <Card 
                key={pkg.name} 
                className={`relative ${pkg.highlight ? "border-green-500 shadow-lg shadow-green-500/25" : "border-gray-700/50 hover:border-green-400/50 transition-colors"}`}
              >
                {pkg.highlight && (
